...
$(document).on("click","#hypdelete",function() {
   ...
   function populateList(){
      $("#customertbl").html('');
      $.ajax({
              url: "http://127.0.0.1:83/api/course/teacher",
              type: 'GET' ,
              headers: { Authorization: 'Bearer '+token},
              success: function(data) {
               console.log(data.data);

               html =    "<table class='table table-striped table-hover'><thead><tr><th>teacher</th><th>Course Code</th><th>Description</th><th>Start Date</th><th>End date</th><th>Actions</th> </tr></thead><tbody>";
               if((data.data)){
                  courseList =  data.data;
                  for (let i = 0; i < courseList.length; i++) { 
                     console.log(courseList[i].name);
                     html = html+ "<tr>"
                     html = html+ "<td>" + courseList[i].email + "</td>" 
